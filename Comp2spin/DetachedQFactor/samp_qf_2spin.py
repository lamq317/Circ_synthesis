...
def run_gate_del_flow_example(in_circuit,
    ...
    dist_tol=1e-5,       # Stopping criteria for distance
    ...
    passes = [
       ...
       synt_pass(layer_generator=SimpleLayerGenerator(two_qudit_gate=VariableUnitaryGate(2),single_qudit_gate_1=VariableUnitaryGate(1)),
                 success_threshold=1e-5,max_layer=5000,instantiate_options=instantiate_options)
    ...
    with Compiler(‘localhost’, default_server_port) as compiler:
...
if __name__ == '__main__':
    ...
    t_idx = 256 if len(sys.argv) < 4 else int(sys.argv[3])
